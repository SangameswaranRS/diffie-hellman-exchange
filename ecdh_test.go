...
func TestKeyAgreementWithMarshal(t *testing.T) {
	dhExchanger, err := NewEllipticCurveExchanger(elliptic.P224())
	require.NoError(t, err)
	require.NotNil(t, dhExchanger)
	...
	// Marshal and reconstruct - BOB
	bobPublicMarshalled, err := bobExchanger.MarshalPublic(bobPublic)
	require.NotNil(t, bobPublicMarshalled)
	require.NoError(t, err)
	bobPublicReconstructed, err := bobExchanger.UnMarshalPublic(bobPublicMarshalled)
	require.NoError(t, err)
	require.NotNil(t, bobPublicReconstructed)
	require.Equal(t, bobPublic, bobPublicReconstructed)

	// Marshal and reconstruct - Alice
	alicePublicMarshalled, err := dhExchanger.MarshalPublic(alicePublic)
	require.NotNil(t, alicePublicMarshalled)
	require.NoError(t, err)
	alicePublicReconstructed, err := dhExchanger.UnMarshalPublic(alicePublicMarshalled)
	require.NoError(t, err)
	require.NotNil(t, alicePublicReconstructed)
	require.Equal(t, alicePublic, alicePublicReconstructed)
	fmt.Println(alicePublicReconstructed, alicePublic)

	aliceCompute, err := dhExchanger.AgreeOnSecret(alicePrivate, bobPublicReconstructed)
	require.NoError(t, err)

	bobCompute, err := bobExchanger.AgreeOnSecret(bobPrivate, alicePublicReconstructed)
	require.NoError(t, err)
